...
export default function useMaze(defaultWidth, defaultHeight) {
  ...
  const [maze, setMaze] = React.useState(
    Array.from(
      { length: size.current.width * size.current.height },
      () => defaultCell
    )
  )
  ...
  function generateMaze(currentCellIdx = 0) {
    ...
    if (visitedCells.current.size < maze.length) {
      ...
      const availableDirections = Object.keys(neighbourDirectionsIdx).filter(
        (direction) => {
        ...
        setMaze((prev) =>
          prev.map((cell, idx) => {
            ...
            if (idx === neighbourDirectionsIdx[randomNeighbourDirection]) {
              return {
                ...cell,
                [oppositeDirection[randomNeighbourDirection]]: 0,
              }
            }
            return cell
          })
